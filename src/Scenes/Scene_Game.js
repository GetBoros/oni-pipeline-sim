...
AScene_Game_Clicker.prototype.create = function ()
{
    ...
    // 1.0. Setup camera background viewport:
    this.cameras.main.setBackgroundColor('rgba(0, 0, 255, 0.3)');

    // 2.0. Create Left UI Container (Root Anchor for portrait elements):
    this.Left_Panel_Container = this.add.container(0, 0);

    // 2.1. Create Portrait Frame (Position is LOCAL relative to Left_Panel_Container):
    ...
    // 2.2. Create Portrait Text (Position is LOCAL relative to Left_Panel_Container):
    this.Portrait_Text = this.add.text(0, -70, 'Portrait', { fontSize: '18px', color: '#ffffff' }).setOrigin(0.5);

    // 2.3. Add children to Left Container:
    this.Left_Panel_Container.add([this.Portrait_Frame, this.Portrait_Text]);

    // 3.0. Create Right UI Container (Root Anchor for debug overlay):
    this.Right_Panel_Container = this.add.container(0, 0);

    // 3.1. Create Debug Resolution Text (Aligned to Top-Right inside its container):
    ...
    // 4.0. Execute initial layout positioning:
    this.Update_Layout(init_w, init_h);

    // 5.0. Register global scale/orientation resize event listener:
...
AScene_Game_Clicker.prototype.On_Window_Resize = function (game_size)
{
    ...
    // Recalculate container anchors when window or screen orientation changes:
...
{
    let padding = 20;
    let left_anchor_x = 90; // Frame offset from left edge
    let left_anchor_y = screen_height / 2; // Vertical center

    // 1.0. Reposition Left Anchor Container:
    if (this.Left_Panel_Container)
    {
        this.Left_Panel_Container.setPosition(left_anchor_x, left_anchor_y);
    }

    // 2.0. Reposition Right Anchor Container (Top-Right corner):
    if (this.Right_Panel_Container)
    {
        this.Right_Panel_Container.setPosition(screen_width - padding, padding);
    }

    // 3.0. Refresh Debug Resolution Text:
